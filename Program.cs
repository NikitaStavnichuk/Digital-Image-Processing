    ...
    class Program
    {
        ...
        static void GradationTransformations()
        {
            ...
            using (var image = new Bitmap(imagePath))
            {
                ...
                CreateHistogramImage(image, histogramheight, histogram1path); //гистограмма исходного изображения
                var w = image.Width;
                var h = image.Height;
                ...
                using (var transformedImage = new Bitmap(w, h))
                {
                    for (int i = 0; i < h; ++i)
                    {
                        for (int j = 0; j < w; ++j)
                        {
                            ...
                            transformedImage.SetPixel(j, i, newPixel); //пересчёт пикселей нового изображения
                        }
                    }
                    ...
                    CreateHistogramImage(transformedImage, histogramheight, histogram2path); //гистограмма изображения после градационного преобразования
                }
            }
        }
        ...
        {
            int[] histogram = new int[256]; //хранение частот яркости пикселей
            int maxHeight = 0; //макс высота гистограммы
            for (int y = 0; y < inputImage.Height; y++)
            {
                for (int x = 0; x < inputImage.Width; x++)
                {
                    Color pixel = inputImage.GetPixel(x, y); //берём пиксель
                    int brightness = (pixel.R + pixel.G + pixel.B) / 3; //яркость пикселя
                    histogram[brightness]++; //увеличение соответствующего значения в массиве histogram
                    if (histogram[brightness] > maxHeight)
                    {
                        maxHeight = histogram[brightness]; //макс высота гистограммы
                    }
                }
            }
            ...
            for (int i = 0; i < 256; i++) 
            {
                int lineStartY = histogramHeight; //масштабирование гистограммы по высоте и отрисовка столбцап гистограммы
                int lineEndY = (int)(histogramHeight - histogram[i] * scaleFactor); //на изображении outputImage
            ...
            outputImage.Dispose(); //освобождает память
        }
    }
}